...
window.addEventListener("load", function () {
  ...
  previewImg.addEventListener("mouseover", function (e) {
    ...
    previewImg.addEventListener("mousemove", function (e) {
      ...
      const offsetX = 0.5 * mask.offsetWidth;
      const offsetY = 0.5 * mask.offsetHeight;
      ...
      // (4) 如果x 坐标小于了0 就让他停在0 的位置
      // 遮挡层的最大移动距离
      const maskMax = previewImg.offsetWidth - mask.offsetWidth;
      // console.log(maskMax);
      if (maskX <= 0) {
        maskX = 0;
      } else if (maskX >= maskMax) {
      ...
      // 大图片的移动距离 X Y
      let bigImgX = (maskX * bigImgXMax) / maskMax;
      let bigImgY = (maskY * bigImgYMax) / maskMax;
